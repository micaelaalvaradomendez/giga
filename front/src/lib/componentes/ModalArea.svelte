	...
	async function cargarAreas() {
		...
		try {
			...
			if (response.ok) {
				...
				if (formData.id_area) {
					areas = areas.filter(area => area.id_area !== formData.id_area);
				}
			}
		} catch (error) {
				...
				<div class="form-group">
					...
					{:else}
						<select 
							...
							disabled={isSaving}
						>
							<option value={null}>-- Sin área padre (raíz) --</option>
							{#each areas as area}
					...
					<small class="form-help">
						Seleccione el área padre para establecer la jerarquía organizacional
					</small>
				</div>
						...
						>
							...
							{#each agentes as agente}
								<option value={agente.id_agente}>
									{agente.nombre} {agente.apellido} ({agente.legajo})
								</option>
							{/each}
						</select>
					{/if}
						...
						<div class="agentes-list">
							{#each agentes as agente}
								<label class="agente-checkbox">
									<input 
										type="checkbox" 
										checked={formData.agentes_asignados && formData.agentes_asignados.includes(agente.id_agente)}
										on:change={() => toggleAgenteAsignado(agente.id_agente)}
										disabled={isSaving}
									>
									...
									<div class="agente-info">
										...
										{#if agente.area_actual}
											<small class="area-actual">Área actual: {agente.area_actual}</small>
										{/if}
									</div>
								</label>
							{/each}
